...
const ImageGenerator: React.FC<ImageGeneratorProps> = ({ mode, characters, setCharacters, onStartEditing, onStartAnimating: _onStartAnimating }) => {
  ...
  finalPrompt = `「${prompt}」を、現代的な“きらつや”アニメ調イラストで描いてください。【線】：シャープでクリーンな輪郭線。細部は描き込みつつ、線は整理されている。
...
【表現】：太めのインク線、強い陰影、ハーフトーン、ダイナミックな構図。`
    ...
    {
      name: "ぷっくりシール",
      prompt: `「${prompt}」を、ぷっくりしたステッカー（シール）風を5枚、台紙も描いてください。
【表現】：白フチ、つや、立体感、ポップで可愛いデフォルメ。背景はシンプル台紙。`
    },
    {
      name: "コルクボード写真",
...
【表現】：少し色あせた写真質感、紙の縁、影、ピンやマスキングテープの演出。`
       ...
       {
      name: "浮世絵版画",
      prompt: `「${prompt}」を、江戸時代の木版画（浮世絵）風に描いてください。
【特徴】：平面的な色面、はっきりした輪郭線、版画の摺りムラ、和紙の質感。
【構図】：大胆なトリミング、装飾的な文様、余白の美。`
    },
    {
      name: "水墨画",
      prompt: `「${prompt}」を、水墨画（墨絵）風に描いてください。
【特徴】：墨の濃淡、にじみ、かすれ、余白の美。最小限の線で情景を表現。`
    },
  ];
