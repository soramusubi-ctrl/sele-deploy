...
const LoginScreen: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (!auth || typeof auth.signInWithPopup !== 'function') {
      setError('Firebase認証が設定されていません。管理者にお問い合わせください。');
      return;
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
      <div className="bg-white rounded-3xl p-12 shadow-lg max-w-md w-full text-center">
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center bg-white border-2 border-stone-200 hover:border-stone-300 hover:bg-stone-50 text-stone-700 font-medium py-3 px-6 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
